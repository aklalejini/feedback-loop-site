...
export function MeadForm({ initial, onSubmit, onCancel, submitLabel = "Save batch" }: Props) {
  ...
  return (
    <form
      className="pixel-card rounded-none p-5 grid gap-6"
      onSubmit={(e) => {
        e.preventDefault();
        onSubmit(draft);
      }}
    >
      <div className="grid sm:grid-cols-[180px,1fr] gap-6 items-start">
        ...
        <div className="grid gap-5">
          <Field label="Batch name" htmlFor="name">
            <input
              id="name"
              className="w-full border-2 border-[var(--ink)] rounded-none px-3 py-2 bg-white"
              value={draft.name}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {Object.values(HONEYS).map((h) => (
                <button
                  ...
                  className="opt rounded-none p-2 flex items-center gap-2 text-left"
                >
                  <span
                    aria-hidden
                    className="pixelated inline-block shrink-0 border-2 border-[var(--ink)]"
                    style={{ width: 20, height: 20, background: h.color, imageRendering: "pixelated" }}
                  />
            ...
            <div className="grid grid-cols-2 gap-2">
              {Object.values(YEASTS).map((y) => (
                <button
                  ...
                  className="opt rounded-none p-2 grid gap-1 text-left"
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm font-semibold">{y.strain}</span>
                    <span className="text-[10px] font-mono border-2 border-[var(--ink)] px-1">
          ...
          <Field label="Spices / additions (optional)">
            <div className="flex flex-wrap gap-2">
              {COMMON_SPICES.map((s) => (
                <button
                  ...
                  className="opt rounded-none px-2 py-1 text-xs font-semibold"
                >
            ...
            <input
              className="w-full border-2 border-[var(--ink)] rounded-none px-3 py-2 bg-white mt-2"
      ...
      {/* stats */}
      <div className="grid grid-cols-3 gap-3 text-sm border-t-2 border-[var(--ink)] pt-4">
      ...
      {headspace < vessel.capacityL * 0.05 && !isEmpty ? (
        <p className="text-sm text-red-800 bg-red-50 border-2 border-red-300 rounded-none px-3 py-2">
      ...
      <div className="flex gap-3 justify-end">
        {onCancel ? (
          <button type="button" onClick={onCancel} className="btn-ghost rounded-none px-4 py-2">
            Cancel
          </button>
        ) : null}
        <button type="submit" className="btn rounded-none px-4 py-2">
          {submitLabel}
        </button>
      </div>
    </form>
...
function Field({ label, htmlFor, children }: { label: string; htmlFor?: string; children: React.ReactNode }) {
  return (
    <div className="grid gap-1.5">
      <label htmlFor={htmlFor} className="text-sm font-bold uppercase tracking-wide">{label}</label>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="grid gap-1.5">
      <div className="flex items-baseline justify-between">
        <label htmlFor={id} className="text-sm font-bold uppercase tracking-wide">{label}</label>
        <span className="font-mono text-base">{value.toFixed(1)} {unit}</span>
      </div>
      <input
        id={id}
        type="range"
